...
public class Anagrams {
    ...
    private static final String SPLIT_EXPRESSION = "\\s*[^a-zA-Z0-9]+\\s*";
    ...
    public boolean sentenceAnagrams(String input1, String input2, boolean caseSensitive) {
        String[] splitSentence1 = input1.split(SPLIT_EXPRESSION);
        String[] splitSentence2 = input2.split(SPLIT_EXPRESSION);
        ...
        for (int i = 0; i < splitSentence1.length; i++) {
            for (int j = 0; j < splitSentence2.length; j++) {
                if (checkedWordsSentence2[j]) {
                    // Current word has already been used as anagram of a previous word
                    // in the sentence 
                    continue;
                }
               
                if (wordAnagrams(splitSentence2[j], splitSentence1[i], caseSensitive)) {
